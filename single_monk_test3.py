...
for i in range(n_attemps):
    ...
    model.add_layer(dim_hid, input_dim=dim_in, activation='relu', kernel_initialization=RandomNormalInitialization())
    model.add_output_layer(dim_out, activation='sigmoid', kernel_initialization=RandomNormalInitialization())

    model.compile(0.6, 0.8, 0.0)
    history = model.fit(
