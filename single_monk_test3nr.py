...
for i in range(n_attemps):
    ...
    model.add_layer(dim_out, activation='tanh', kernel_initialization=RandomNormalInitialization())

    model.compile(optimizer=SGD(lr=0.5, mom=0.8, l2=0.0))

    history = model.fit(X_train, Y_train, 200, X_train.shape[0], ts=(X_test, Y_test),
                        verbose=True
                        )
    if i == 0:
