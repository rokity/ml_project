...
model.add_layer(30, input_dim=X_train.shape[1], activation='tanh', kernel_initialization=RandomNormalInitialization())
model.add_layer(40, activation='tanh', kernel_initialization=RandomNormalInitialization())
model.add_layer(Y_train.shape[1], activation='linear', kernel_initialization=RandomNormalInitialization())

model.compile(optimizer=Adam(lr=0.001, beta_1=0.9, beta_2=0.99, l2=0.0004))

model.fit(
    X_train, Y_train, 500, batch_size=16, ts=(X_test, Y_test),
